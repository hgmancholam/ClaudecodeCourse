...
- A single `@container slide (min-aspect-ratio: 1.15)` block switches layout: wide screens get 2-column bodies (`.body.grid2`) and slightly smaller relative type; square/export collapses to one stacked column. This is the only breakpoint — keep new responsive rules inside it rather than adding media queries.
