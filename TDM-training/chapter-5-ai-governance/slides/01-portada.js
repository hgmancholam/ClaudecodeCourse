(window._deck = window._deck || []).push({
en: `
<section class="slide cover s-env" data-title="portada">
  <div class="kicker">Chapter 5 — Delivery Control</div>
  <div class="body">
    <h1>AI-Enabled <span class="accent">Governance</span></h1>
    <div class="lead">Tighten how work starts and finishes, and measure progress that's actually real.</div>
  </div>
  <footer><span class="brand">AI Capabilities Training <span class="dot">·</span> Delivery Lead</span><span class="author">Giovanny Manchola</span><span>2026</span></footer>
</section>
`,
es: `
<section class="slide cover s-env" data-title="portada">
  <div class="kicker">Capítulo 5 — Control del Delivery</div>
  <div class="body">
    <h1>Gobernanza <span class="accent">habilitada por IA</span></h1>
    <div class="lead">Afina cómo empieza y termina el trabajo, y mide el avance que de verdad es real.</div>
  </div>
  <footer><span class="brand">AI Capabilities Training <span class="dot">·</span> Delivery Lead</span><span class="author">Giovanny Manchola</span><span>2026</span></footer>
</section>
`
});
